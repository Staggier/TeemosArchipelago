...
func _init() -> void:
	...
	state_machine.add_state("pause", PauseState.new(game.get_child(6).get_child(3) as PauseMenu, state_machine))
	state_machine.add_state("controls", ControlsState.new(game.get_child(6).get_child(2) as ControlsMenu, state_machine))
	state_machine.current_state = state_machine.states["play"]
	
	# Give Menus access to the Game's state machine
	(game.get_child(6).get_child(2) as ControlsMenu).state_machine = state_machine
	(game.get_child(6).get_child(3) as PauseMenu).state_machine = state_machine
	
	# Give Pause Menu access to Main Menu
	(game.get_child(6).get_child(3) as PauseMenu).main_menu = main_menu
